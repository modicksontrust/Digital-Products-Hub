...
import { CheckCircle2, ChevronDown, ShieldCheck, ArrowRight, Star, BookOpen } from "lucide-react";
...
export interface SalesPagePreviewProps {
  headline: string;
  subheadline: string;
  bullets: string[];
  whoItsFor: string;
  ...
  /**
   * When provided the CTA buttons are interactive and call this handler.
   * When omitted (preview mode) the buttons render as non-interactive.
   */
  onBuy?: () => void;
}

/**
 * Shared presentational component used both by the public buyer-facing
 * SalesPage (which supplies `onBuy`) and by the editor live preview inside
 * CreateEbook step 8 (which omits `onBuy`, making buttons non-interactive).
 * All visual markup lives here — keep SalesPage.tsx as a thin data shell.
 */
export function SalesPagePreview({
  ...
  const subtitleText = subheadline;
  const buttonText = ctaText || "Get the eBook";

  const isPreview = !onBuy;
        ...
        >
          ...
          <div className="space-y-4">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-serif font-medium leading-[1.15] text-[#1A1E27]">
              {titleText || (isPreview ? <span className="opacity-30 italic">Your headline will appear here</span> : null)}
            </h1>
            {subtitleText && (
          ...
          {authorName && (
            <p className="text-base text-[#20242E] border-l-2 border-[#B8863B] pl-4 italic inline-block">
              By <span className="font-semibold not-italic">{authorName}</span>
            </p>
          )}
          ...
          <div className="pt-4 flex flex-col sm:flex-row items-center md:items-start gap-4">
            <Button
              onClick={onBuy}
              className={cn(
                "w-full sm:w-auto h-auto min-h-12 px-7 py-3 text-base font-bold bg-[#B8863B] hover:bg-[#A6742D] text-white rounded-none shadow-[0_10px_30px_rgba(184,134,59,0.25)] hover:shadow-[0_10px_36px_rgba(184,134,59,0.35)] transition-all duration-300 whitespace-normal text-center leading-snug",
                isPreview && "pointer-events-none"
              )}
            >
              {buttonText} — {formatPrice(priceCents)}
            </Button>
      ...
      >
        <div className="max-w-6xl mx-auto px-6 py-6 flex flex-wrap items-center justify-center gap-x-12 gap-y-4 text-[#6B7284]">
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium text-[#20242E] uppercase tracking-wider text-xs">{chapterCount ?? "—"} Chapters</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="grid sm:grid-cols-2 gap-5 md:gap-6">
              {bullets.map((bullet, i) => (
                ...
                >
                  ...
                  <p className="text-[#4B5266] leading-relaxed group-hover:text-[#20242E] transition-colors text-base">
                    {bullet || (isPreview ? <span className="opacity-40 italic">Bullet point</span> : null)}
                  </p>
                </motion.div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {faq.map((item, i) => (
                ...
                >
                  ...
                  >
                    <span className="font-semibold text-base text-[#20242E] group-hover:text-[#96692A] transition-colors">
                      {item.question || (isPreview ? <span className="opacity-40 italic">Question</span> : null)}
                    </span>
                    ...
                    {openFaq === i && (
                      ...
                      >
                        <div className="px-5 md:px-6 pb-6 text-[#4B5266] leading-relaxed text-base border-t border-[#E7E1D4] pt-5 mt-2">
                          {item.answer || (isPreview ? <span className="opacity-40 italic">Answer</span> : null)}
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
        ...
        <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center gap-5">
              <div className="text-3xl md:text-4xl font-serif text-[#1A1E27] mb-1">{formatPrice(priceCents)}</div>
              <Button
                onClick={onBuy}
                className={cn(
                  "w-full sm:w-auto min-w-[260px] h-auto min-h-14 px-8 py-4 text-lg font-bold bg-[#B8863B] hover:bg-[#A6742D] text-white rounded-none shadow-[0_10px_30px_rgba(184,134,59,0.25)] hover:shadow-[0_10px_36px_rgba(184,134,59,0.35)] transition-all duration-300 whitespace-normal text-center leading-snug",
                  isPreview && "pointer-events-none"
                )}
              >
                {buttonText} <ArrowRight className="ml-3 w-5 h-5 shrink-0 inline" />
              </Button>
            </div>
          </motion.div>
        </div>
